...
double get_infomap_partition_from_edge_list(int Ntrials, int random_seed, \
    ...
    */
    ...
    string config_str = string(num_trials_ch) + string(seed_ch) + " --two-level --undirected  ";
    
    
    infomap::Config config = infomap::init(config_str);
	infomap::Network network(config);
    
    // fill in the network
    for(map<pair<int, int>, double>::iterator itm=edges.begin(); itm!=edges.end(); itm++) {
        network.addLink(itm->first.first, itm->first.second, itm->second);
    }
    ...
    // get hard_memberships = {old_label : cluster}
    hard_memberships.clear();

    for (infomap::LeafIterator leafIt(&resultNetwork.getRootNode()); !leafIt.isEnd(); ++leafIt) {
        
        int lnode = leafIt->originalLeafIndex;
        infomap::SNode* parent = leafIt->parentNode;
        while (parent->parentNode != & resultNetwork.getRootNode()) {
            parent = parent->parentNode;
        }
        
        int module_from_infomap = parent->parentIndex;
        //hard_memberships[new_labels.at(lnode)] = module_from_infomap;
        hard_memberships[new_labels.at(lnode)] = module_from_infomap;
