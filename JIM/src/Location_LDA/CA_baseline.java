...
/**
 * similar as ILA_LDA but use item category 
 * instead of item location
 * 
 * 
 * @author huzhiting
 *
...
public class CA_baseline {
  ...
  */
    ...
    public CA_baseline(int V, int K, int U, int C, double[] alpha, /*double[] alpha_2,*/
    	...
    	for(int i=0; i<U; i++){
    		...
    		for(int j=0; j<length; j++){
    			...
    			topicItemCountSum[ran]++;
       			...
       			topicLocCount[ran][user_item.get(i).getItemCategory(j)]++;
 ...
 /*   
    ...
    public int sample_s(int i, int j){
		...
		for(int u=0; u<2; u++){
			...
			if(u == 0){
				first_term = 
					(locTopicCount[i][topic] + alpha_2[topic]) / (locTopicCountSum[i] + alpha_2_sum);
 ...
 */
    ...
    public int sample_z(int i, int j){
    	...
    	int itemLoc = user.getItemCategory(j);
	...
	*/
		...
		/*	
			if(s == 0){
				first_term = 
					(locTopicCount[i][u] + alpha_2[u]) / (locTopicCountSum[i] + alpha_2_sum);
		...
		*/
		...
		// test
		if(u >= K){
			System.err.println("Sample z " + p[K-1] + " " + t);
		}
