...
import Location_LDA.ULA_LDA.pair;

public class UILA_LDA {
	public class pair{
		public int a, b;
    ...
    public int M; //The number of user locations
    public int L;//The number of item locations
    ...
    public int[][] topicLocCount;
    public int[] topicLocCountSum;
  ...
  */
    ...
    // [z][l]: topic - loc
    public double[][] topicLocDistribution;
    // s = 1 : user
    ...
    public UILA_LDA(int V, int K, int U, int M, int L, double[] alpha, double[] alpha_2,
        ...
        topicLocCount = new int[K][L];
        topicLocCountSum = new int[K];
    	...
    	for(int i=0; i<U; i++){
    		...
    		for(int j=0; j<length; j++){
    			...
    			topicItemCountSum[ran]++;
       			...
       			topicLocCount[ran][user_item.get(i).getItemLoc(j)]++;
    			topicLocCountSum[ran]++;
    			
    			if(user_item.get(i).getS(j) == 0){
    				locTopicCount[user_item.get(i).location][ran]++;
    				locTopicCountSum[user_item.get(i).location]++;
    			}
    			else{
    ...
    public void updateParameter(){    
        ...
        for(int i=0; i<K; i++){
        	for(int j=0; j<L; j++){
        		topicLocSum[i][j] +=
        			(topicLocCount[i][j] + beta_2[j])
        			/ (topicLocCountSum[i] + beta_2_sum);
        	}
        }
        topicLocNum++;
...
//beta prior distribution for branch selection
    ...
    public pair sample(int i,int j)
    {
		...
		topicLocCount[topic][itemLoc]--;
		topicLocCountSum[topic]--;
		...
		for(int st=0;st<2;st++)
			for(int z=0;z<K;z++)
			{
				...
				if(st == 0){
					...
					fourth_term =
							(topicLocCount[z][itemLoc] + beta_2[itemLoc]) / (topicLocCountSum[z] + beta_2_sum); 
				}
				else{
					first_term = 
		...
		topicLocCount[p.b][itemLoc]++;
		topicLocCountSum[p.b]++;
    	
    	return p;
    }
    
    
    //uniform prior distribution for branch selection
    public pair sample2(int i,int j)
    {
    	pair p=new pair();
    	UserProfile user = user_item.get(i);
    	int loc = user.location;
		...
		for(int st=0;st<2;st++)
			for(int z=0;z<K;z++)
			{
				double first_term,third_term,fourth_term;	
				if(st == 0){
					//System.out.println(i+" "+topic);
				...
				probability[st][z]=first_term*third_term*fourth_term;
			}
    ...
    public int sample_z(int i, int j){
		...
		for(int u=0; u<K; u++){
			...
			if(s == 0){
				...
				third_term =
					(topicLocCount[u][itemLoc] + beta_2[itemLoc]) / (topicLocCountSum[u] + beta_2_sum); 
			}
			else{
				first_term = 
    ...
    public void calDistribution(){	     
     ...
     for(int i=0; i<K; i++){
     	for(int j=0; j<L; j++){
     		topicLocDistribution[i][j] =
     			topicLocSum[i][j] / (double)topicLocNum;
     	}
     }
  ...
  */
    ...
    try{
        ...
        // topicLoc
    	String topicLoc_file = base_path + "topicLoc.txt";
    	oswpf = data_storage.file_handle(topicLoc_file);
    	oswpf.write(K+" "+L+"\n");
        for(int i=0; i<K; i++){
        	for(int j=0; j<L; j++){
        		oswpf.write(topicLocDistribution[i][j]+" ");
        	}
        	oswpf.write("\n");
        }
		oswpf.flush();
        ...
        for(int i=0; i<U; i++){
        	...
        	System.out.println("lambda_u:"+lambda_u[i]);
        }
    	oswpf.write("\n");
		oswpf.flush();
		oswpf.close();
