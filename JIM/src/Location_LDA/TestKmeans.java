...
58.     */  
   public static void main(String[] args) {  
        ...
        String file="user_location.csv";
        TestKmeans cluster = new TestKmeans(file);  
       
        cluster.ClassCount=20;
        cluster.FieldCount=1;
        cluster.innitialize();
        System.out.println("case number:"+InstanseNumber);
       //读取数据   TestKmeans
          cluster.readData(file);  
        //聚类过程  
        long startTime = System.currentTimeMillis();  
        cluster.cluster();  
        //输出结果   
        cluster.printResult("movielens_users2.csv");  
    ...
    {  
       ...
       {  
           ...
           for(int j = 0;j < classData.length;j++){  
              //欧式距离   
               float newMin = calDis(data[i],classData[j]);  
